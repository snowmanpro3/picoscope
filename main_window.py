...
class Picoscope_thermocouple_GUI(QMainWindow):
    ...
    def connect_to_pico(self):
        if self.gui.connect_button.text() == "CONNECT":
            ...
            self.pico = pico5000(log_func=self.dual_print) #???????? НЕ ПОНИМАЮ
    ...
    def connect_ui_elements(self):
        ...
        self.gui.meas_time_input.textChanged.connect(lambda text: self.params.update({'meas_time': int(text.strip())}))
        ...
        self.gui.t_treshold_input.textChanged.connect(lambda text: self.params.update({'t_treshold': int(text.strip())}))
        self.gui.t_direction_choice.currentTextChanged.connect(lambda text: self.params.update({'t_direction': text.strip()}))
        self.gui.t_delay_input.textChanged.connect(lambda text: self.params.update({'t_delay': int(text.strip())}))
        self.gui.t_auto_time_input.textChanged.connect(lambda text: self.params.update({'t_auto_time': int(text.strip())}))
    ...
    def set_default_values(self):
        ...
        self.gui.meas_time_input.setText("100") #! Го миллисекунды
        ...
        self.gui.t_direction_choice.setCurrentText("RISING")
        self.gui.dt_limit_input.setText("0.0005")
